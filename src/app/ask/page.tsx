...
export default function AskQuestion() {
  ...
  const [maulanaId, setMaulanaId] = useState(""); // "" means auto-assign
  const [maulanas, setMaulanas] = useState<any[]>([]);
  const [loading, setLoading] = useState(false);
  const [submitting, setSubmitting] = useState(false);
  const [success, setSuccess] = useState(false);

  useEffect(() => {
    async function fetchMaulanas() {
      setLoading(true);
      try {
        const res = await fetch("/api/maulanas");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/questions", {
        ...
        body: JSON.stringify({ title, content, maulanaId: maulanaId || undefined }),
      });
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-brand-sand px-6">
        <div className="glass-panel p-12 text-center max-w-lg animate-in zoom-in-95 duration-500">
           ...
           <p className="text-gray-600">Your question has been successfully submitted and assigned to a Maulana. You will be redirected to the forum shortly.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="flex items-center gap-4 mb-8">
           ...
           <div>
             ...
             <p className="text-gray-500">Seek guidance from our approved community of Maulanas.</p>
           </div>
        </div>
          ...
          <div className="glass-panel p-8 space-y-6 shadow-sm border-0">
            ...
            <div className="space-y-2">
              ...
              <textarea 
                ...
                placeholder="Please provide details to help the Maulana understand your context..." 
            ...
            <div className="space-y-4">
              <label className="text-sm font-bold text-brand-dark uppercase tracking-wider">Assign to Maulana</label>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                 <button 
                   type="button"
                   onClick={() => setMaulanaId("")}
                   className={`p-4 rounded-xl border flex items-center gap-3 transition-all ${maulanaId === "" ? "border-brand-gold bg-brand-gold/5 shadow-sm" : "border-gray-100 bg-white hover:border-gray-200"}`}
                 >
                   <Shuffle className={maulanaId === "" ? "text-brand-gold" : "text-gray-400"} size={20} />
                   <div className="text-left">
                     <p className="text-sm font-bold">Auto-Assign</p>
                     <p className="text-xs text-gray-400">Least busy Maulana available</p>
                   </div>
                 </button>
                 ...
                 <div className="relative">
                    <select 
                      value={maulanaId}
                      onChange={(e) => setMaulanaId(e.target.value)}
                      className={`w-full h-full px-4 py-3 pr-10 rounded-xl border outline-none transition-all appearance-none ${maulanaId !== "" ? "border-brand-gold bg-brand-gold/5" : "border-gray-100 bg-white hover:border-gray-200 text-gray-400"}`}
                    >
                      <option value="">Manual Select...</option>
                      {loading ? (
                        <option disabled>Loading Maulanas...</option>
                      ) : (
                        maulanas.map((m) => (
                          <option key={m.id} value={m.id}>{m.name || "Unknown Maulana"}</option>
                        ))
                      )}
                    </select>
                    <UserCheck className={`absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none ${maulanaId !== "" ? "text-brand-gold" : "text-gray-300"}`} size={20} />
                 </div>
              </div>
            </div>
          </div>
